...
export default function ControlCenterMenu({
  ...
  const { dark, wifi, brightness, keyboardBrightness, bluetooth, airdrop, fullscreen, volume } = useStore(
    (state) => ({
      dark: state.dark,
      wifi: state.wifi,
  ...
  const toggleKeyboardBrightness = () => {
    const nextValue = keyboardBrightness >= 95 ? 0 : Math.min(keyboardBrightness + 25, 100);
    setKeyboardBrightness(nextValue);
  };

  const keyboardLevel =
    keyboardBrightness === 0 ? "Off" : `${Math.round(keyboardBrightness)}%`;

  return (
    <div
      className="w-80 h-[29rem] max-w-full shadow-menu p-2.5 text-c-black bg-c-100/70"
      pos="fixed top-9.5 right-0 sm:right-1.5"
      border="~ menu rounded-2xl"
      grid="~ cols-4 rows-6 gap-2"
    ...
    >
      ...
      <div
        ...
        onClick={toggleKeyboardBrightness}
      >
        <span className="i-bi:brightness-alt-high text-xl" />
        <span text="xs center" font="leading-3.5">
          Keyboard Glow
        </span>
        <span className="mt-1" text="xs c-500 center">
          UI only · {keyboardLevel}
        </span>
      </div>
      <div
        className="cc-grid flex-center flex-col cursor-default"
      ...
      <div className="cc-grid col-span-4 px-2.5 py-2 space-y-1 flex flex-col justify-around">
        <span className="font-medium ml-0.5">Keyboard Glow</span>
        <SliderComponent
          icon="i-bi:brightness-alt-high"
          value={keyboardBrightness}
          setValue={setKeyboardBrightness}
        />
      </div>
      <div className="cc-grid col-span-4 px-2.5 py-2 space-y-1 flex flex-col justify-around">
